...
`change_log` / `comments`, if present, are used only to try to derive a
reviewer list per patch (see `extract_reviewers`) — if your schema stores
reviewers differently, adjust that one function.
...
def build_features(df, emb, i, j):
    ...
    feats = {
        **file_stats,
        ...
        "same_reviewer": float(bool(row_i.get("reviewers", set()) & row_j.get("reviewers", set()))),
    }
    return feats
...
def load_or_compute_embeddings(df, sbert_model_name, cache_path):
    ...
    if cache_path:
        ...
        np.save(cache_path, {"model_name": sbert_model_name, "n_rows": len(df), "embeddings": emb})
...
def load_ground_truth(path, df):
    id_to_idx = {str(cid): i for i, cid in enumerate(df["patch_id"])}
    raw_pairs = []
...
def build_training_rows(df, emb, anchor_positive_map, window_days, max_negatives, hard_ratio, seed):
...
def evaluate(model, df, emb, anchors, window_days, feature_cols, top_k, positives_by_anchor):
    ...
    for anchor_idx in anchors:
        ...
        feats = pd.DataFrame([build_features(df, emb, anchor_idx, c) for c in candidate_idxs])
...
def main():
    ...
    print("Encoding titles + descriptions with SBERT...")
    emb = load_or_compute_embeddings(df, args.sbert_model, args.embedding_cache)
    ...
    train_groups = build_training_rows(df, emb, train_map, args.window_days, args.max_negatives, args.hard_negative_ratio, args.seed)
    test_groups = build_training_rows(df, emb, test_map, args.window_days, args.max_negatives, args.hard_negative_ratio, args.seed + 1)

    X_train, y_train, group_sizes_train = groups_to_frame(df, emb, train_groups)
    X_test, y_test, group_sizes_test = groups_to_frame(df, emb, test_groups)
    ...
    mrr, recall_k, n_eval = evaluate(model, df, emb, test_anchors, args.window_days, feature_cols, args.top_k, positives_by_anchor)
    print(f"  MRR:         {mrr:.4f}")
