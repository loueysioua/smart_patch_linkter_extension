...
def _entry_username(entry):
    """Pulls a username out of a change_log/comment entry's nested
    `author` object (ResolvedAccount: account_id, name, display_name,
    email, username), per the Gerrit types schema."""
    if not isinstance(entry, dict):
        return None
    author = entry.get("author")
    if not isinstance(author, dict):
        return None
    val = author.get("username") or author.get("email") or author.get("name")
    return str(val).lower() if val else None


def extract_reviewers(row):
    """Reviewer/commenter usernames from change_log + comments entries."""
    reviewers = set()
    for entry in (row.get("change_log_parsed") or []):
        uname = _entry_username(entry)
        if uname:
            reviewers.add(uname)
...
def extract_discussion_text(row):
    """Text blob from change_log + comments `message` fields, for SBERT
...
# Generic bug/ticket references not tied to Gerrit's own linking convention
# (e.g. an external tracker). Covers "BUG-1234", "#4821", "Bug: 4821",
# "BUG=4821" (the last two are common Chromium/Google-style footers).
TICKET_RE = re.compile(
    r"\b([A-Z][A-Z0-9]{1,9}-\d+|#\d{3,7})\b|\bBUG[:=]\s*(\d{3,8})\b",
    re.IGNORECASE,
)
...
# Gerrit's own structural linking convention (see GerritLinkage in the types
# file): a full Change-Id hash, a Depends-On/Related-To/Cherry-picked-from
# trailer, or a change URL/number. These are much stronger evidence of an
# intentional link than free-text similarity, so they're split into a
# "strong" tier (hash-level match) and a "weak" tier (URL/bare-number match,
# which can be noisier - mirrors GerritLinkage's own confidence tiers).
CHANGE_ID_RE = re.compile(r"\bI[0-9a-f]{40}\b")
DEPENDS_ON_RE = re.compile(r"(?:Depends-On|Related-To|Cherry-picked-from)\s*:\s*(\S+)", re.IGNORECASE)
GERRIT_URL_RE = re.compile(r"https?://\S+?/c/\S+?/\+/(\d+)")
BARE_NUMBER_TRIGGER_RE = re.compile(r"(?:change|patch|cl)\s*#?\s*(\d{3,7})", re.IGNORECASE)


def extract_gerrit_refs(text):
    strong, weak = set(), set()
    if not text:
        return strong, weak
    strong.update(m.upper() for m in CHANGE_ID_RE.findall(text))
    for m in DEPENDS_ON_RE.findall(text):
        strong.add(m.strip().upper())
    for m in GERRIT_URL_RE.findall(text):
        weak.add(f"NUM:{m}")
    for m in BARE_NUMBER_TRIGGER_RE.findall(text):
        weak.add(f"NUM:{m}")
    return strong, weak
...
def build_features(df, emb, emb_disc, i, j, use_reference_features=False):
    ...
    gerrit_strong_i = row_i.get("gerrit_refs_strong", set())
    gerrit_strong_j = row_j.get("gerrit_refs_strong", set())
    gerrit_weak_i = row_i.get("gerrit_refs_weak", set())
    gerrit_weak_j = row_j.get("gerrit_refs_weak", set())
    ...
    # Off by default: explicit Change-Id / Depends-On / bug-ticket references
    # are close to (or literally) the mechanism that defines a "linked"
    # patch. Including them lets the model answer by lookup instead of by
    # learning genuine title/description/file/discussion similarity, which
    # defeats the point of a similarity model for the cases that actually
    # need one (patches with no explicit marker). Pass
    # use_reference_features=True only for a deliberate side-by-side
    # comparison, not for the model you intend to rely on.
    if use_reference_features:
        feats["shares_ticket_ref"] = float(bool(ticket_refs_i & ticket_refs_j))
        feats["shares_gerrit_ref_strong"] = float(bool(gerrit_strong_i & gerrit_strong_j))
        feats["shares_gerrit_ref_weak"] = float(bool(gerrit_weak_i & gerrit_weak_j))
    return feats
...
def load_dataset(csv_path):
    ...
    df["change_log_parsed"] = df["change_log"].apply(safe_parse_list) if "change_log" in df.columns else [[] for _ in range(len(df))]
    df["comments_parsed"] = df["comments"].apply(safe_parse_list) if "comments" in df.columns else [[] for _ in range(len(df))]
    ...
    full_text = df["title"] + " " + df["description"] + " " + df["discussion_text"]
    df["ticket_refs"] = full_text.apply(extract_ticket_refs)
    gerrit_refs = full_text.apply(extract_gerrit_refs)
    df["gerrit_refs_strong"] = gerrit_refs.apply(lambda t: t[0])
    df["gerrit_refs_weak"] = gerrit_refs.apply(lambda t: t[1])
...
def report_gerrit_ref_signal(df, pairs):
    """Diagnostic: how many ground-truth pairs already carry an explicit
    structural reference (Change-Id / Depends-On / ticket). If this is a
    large fraction, be aware the ranker can partially "solve" those pairs
    with a single feature lookup rather than learning semantic/file
    similarity - which is fine for retrieval, but means MRR/Recall@k on
    the full test set may be dominated by these easy cases. Worth knowing,
    especially if your ground truth was itself built from this kind of
    reference detection."""
    strong, weak, ticket = 0, 0, 0
    for i, j in pairs:
        if df.loc[i, "gerrit_refs_strong"] & df.loc[j, "gerrit_refs_strong"]:
            strong += 1
        if df.loc[i, "gerrit_refs_weak"] & df.loc[j, "gerrit_refs_weak"]:
            weak += 1
        if df.loc[i, "ticket_refs"] & df.loc[j, "ticket_refs"]:
            ticket += 1
    n = len(pairs)
    print(f"  {strong}/{n} ({100*strong/n:.1f}%) of pairs share a strong Gerrit reference (Change-Id / Depends-On)")
    print(f"  {weak}/{n} ({100*weak/n:.1f}%) share a weak Gerrit reference (URL / bare number)")
    print(f"  {ticket}/{n} ({100*ticket/n:.1f}%) share a generic ticket reference (BUG=, JIRA-style, #1234)")
    if strong / n > 0.5:
        print("  \u26a0\ufe0f  over half your labeled pairs already carry an explicit structural reference. "
              "Consider checking MRR/Recall@k separately on pairs WITH vs WITHOUT one of these references, "
              "to see how much the semantic/file-similarity features are actually contributing.")
...
def groups_to_frame(df, emb, emb_disc, groups, use_reference_features=False):
    feat_rows, labels, group_sizes = [], [], []
    for anchor_idx, cand_idxs, cand_labels in groups:
        if not cand_idxs:
            continue
        for cidx, lbl in zip(cand_idxs, cand_labels):
            feat_rows.append(build_features(df, emb, emb_disc, anchor_idx, cidx, use_reference_features))
            labels.append(lbl)
...
def evaluate(model, df, emb, emb_disc, anchors, window_days, feature_cols, top_k, positives_by_anchor, use_reference_features=False):
    ...
    for anchor_idx in anchors:
        ...
        feats = pd.DataFrame([build_features(df, emb, emb_disc, anchor_idx, c, use_reference_features) for c in candidate_idxs])
...
def main():
    ...
    parser.add_argument("--use-reference-features", action="store_true",
                         help="Include shares_ticket_ref / shares_gerrit_ref_strong / shares_gerrit_ref_weak "
                              "as training features. OFF by default: these encode explicit Change-Id / "
                              "Depends-On / bug-ticket references, which is close to (or literally) how "
                              "linked patches get identified in the first place - including them lets the "
                              "model answer by lookup instead of learning genuine similarity. Only enable "
                              "this for a deliberate side-by-side comparison run.")
    ...
    print("Checking explicit-reference signal in ground truth...")
    report_gerrit_ref_signal(df, pairs)
    print(f"  reference features (shares_ticket_ref / shares_gerrit_ref_*) will be "
          f"{'INCLUDED' if args.use_reference_features else 'EXCLUDED'} in training "
          f"({'--use-reference-features was set' if args.use_reference_features else 'default: off, to avoid the model just looking up an explicit link marker'})")
    ...
    X_train, y_train, group_sizes_train = groups_to_frame(df, emb, emb_disc, train_groups, args.use_reference_features)
    X_test, y_test, group_sizes_test = groups_to_frame(df, emb, emb_disc, test_groups, args.use_reference_features)
    ...
    mrr, recall_k, n_eval = evaluate(model, df, emb, emb_disc, test_anchors, args.window_days, feature_cols, args.top_k, positives_by_anchor, args.use_reference_features)
    print(f"  MRR:         {mrr:.4f}")
    ...
    joblib.dump({
        ...
        "use_reference_features": args.use_reference_features,
    }, out_path)
